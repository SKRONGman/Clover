...
function railHtml(){
  ...
  return head+`<div class="legs">${rows}</div>`+(RAIL_EDIT?pairsHtml(S.legs,"rail"):"")
    ...
    +(S.placed?`<p class="stamp">${S.placed.voided?"Voided":S.placed.saved?"Saved #"+S.placed.saved.no:"Placed"} · ${esc(S.placed.who)} · ${money(S.placed.pays)} on $1</p>`:"")
...
function renderRail(){
  ...
  const c=box.querySelector("#railClear"); if(c) c.onclick=()=>{ S.legs=[]; S.pays=null; S.placed=null; REC.ui=""; RAIL_EDIT=false; render(); };
