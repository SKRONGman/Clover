...
## 2026-09-20 (night) — row 1: the safety net
- **`refresh.py` split** (55.7 KB -> 18.8 KB) into `cfbd.py`, `nfl.py`, `odds.py`, `ratings_math.py`, `common.py`. Code was moved by script, not retyped; old and new were run against the same fake feeds and produced a byte-identical slate. New modules were pushed first and `refresh.py` last, so no hourly run could land on a half-split repo. Every push md5-verified.
- **The outage fix.** `cfbd.get()` no longer calls `sys.exit`: it retries timeouts and 5xx (3 tries), never retries a 401/429, and raises `CFBDDown`. `write_upcoming` catches it, carries college forward (`carry_college`; a carried game that has kicked off becomes `live` and loses its table), and still pulls the NFL. After the first fatal failure the rest of the run doesn't call CFBD again. The NFL side got the same treatment: `pull_nfl` returns `(games, fresh)` and a feed that is down is no longer confused with an empty week.
- **Truthful clock.** `R.asof[league]` = last fresh pull; `R.stale[league]` = `{since, reason}` while carried forward; `lines_generated` only moves on a fresh pull. Page: the Last refresh tag appends "API ISSUE — College lines last updated <time>" in red, reason on hover (Danny's wording, chosen in chat).
- **`health.json`** + `python refresh.py --health-gate`. CFBD calls left come from CFBD's own `GET /info` (`remainingCalls`), asked at most every 6 h because it may itself cost a call; Clover's own count is the fallback (it only knows calls since it started counting, so it overstates what is left until the 1st). A 429 sets calls left to 0. Alerts: stale league, CFBD < 500, odds < 1,000. `alert_now` is true once per UTC day.
- **Fewer calls.** `/rankings` only for weeks not already in `R.rankings` (the weekly full run re-asks all); kept out of `ratings.js` (`PAGE_SKIPS`). ESPN: one attempt instead of three, and only the status code is recorded (the 850 B error page is gone from `nfl_diag`). Team info was already cached per season; `cached()` now refuses to save an empty answer.
- **Bug the audit missed:** a full (Tuesday) run built a blank record, so `upcoming` was empty when `write_upcoming` looked for alt lines and frozen percentages to carry over - both vanished until Thursday's alt pull. Fixed with `CARRY_KEYS`. A full run with CFBD down now falls back to last week's ratings and refreshes lines only.
- **`ci/`**: `checks.py` (size guard: fail 24,800 B - the largest file ever pushed intact was 24,767 B - warn 20,000 B; stale-word counts that may only go down; Python tests; page test), `test_refresh.py` (12 tests on a fake internet, one replaying the 429 outage), `fixture.py` (a slate dated relative to now, so tests work off-season), `smoke.js` (boots `preview1-3.js` in Node's `vm` with a stand-in DOM; checks page % = Python's frozen % to 0.00006, hot slips 2-6 in both leagues, a hand-built slip, My Bet, the line sheet, the verdict scale; a file truncated at 11 KB fails it).
- **Workflow files written, not yet committed** (connector cannot write `.github/workflows/`): new thin `ci.yml`; `refresh.yml` gets actions pinned by SHA (`actions/checkout` 11d5960a = v4.4.0, `actions/setup-python` a26af69b = v5.6.0, the versions the moving tags pointed at that night), a page boot against the fresh `ratings.js` before the commit, `health.json` in the `git add`, `git pull --rebase` on a push race, and the health gate as the last step.
- **Not done, on purpose:** `index.html` dead CSS (17 rules, 1.9 KB) and the 3 unused imports are built but not pushed - `calibrate.py` is 24.5 KB and should be split the next time it changes. `NFL_ALT_EVERY_OTHER_WEEK` unchanged pending Danny's answer.
