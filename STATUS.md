...
Last updated: 2026-09-21. **Rows 1 and 2 are done and live. Row 3 (design direction) is decided. Row 4's mockup is approved with all four calls ruled (see `DECISIONS.md`, rulings of 2026-09-21); build starts next chat. Nothing on the live page changed.**
...
Kick off the next chat with: **"Row 4: build it."** The mockup is approved; the chat opens with the build plan (files, push order) and then pushes.
...
4. **Front door - MOCKUP APPROVED 2026-09-21, build next.** Mockup (the first frame; frame 2 is superseded): https://claude.ai/artifact/PCbU7P1A4UbhYm4tmvcmCn - real lines and chances, the page's own slip search, pretends it is Saturday 9/26 10 AM. Danny's four rulings are in `DECISIONS.md`; the team color rule is in `DESIGN.md`. Build needs: `refresh.py` saves a first-seen line per game (DK only, carried like `CARRY_KEYS`); page files rebuilt in the Board look (`index.html`, `clover.css`, `filters.js`, `preview2.js`, `preview3.js`; `preview1.js` only for the swap rule in `toggleLeg`); new checks for swap, N/A hidden until edit, moved flag only at 3+ and only in the rail. Found while mocking: NFL teams have no `colors2`. The row as planned: In the dark Board look: folded filters, today's upcoming games, Hot Slips always open with picks-only tickets, My Bet rail, line-only spread/total buttons, line-moved flag (if Danny approves it). The mockup must also settle where the one-tap N/A and Prohibited buttons live now that tickets no longer show game grids, and the too-dark team color cutoff. Clickable mockup first.
